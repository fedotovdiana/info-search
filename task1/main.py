...
# output
# - 100 text files with content (>= 1000 words each) of a page
# - index.txt with map of (page number):(page web address)

# algorithm
# 1. get content of page by input web urls
# 2. extract all links <a href="..." /a>
# 3. clear all html tags
# 4. if content of a page >= 1000 words then save it to file (page number):(page web url)
# 5. while number of saved pages < 100: repeat for each reference

# error notifications:
# - invalid input web url
# - page tree may contain less then 100 suited pages
...
if __name__ == '__main__':
    ...
    now = datetime.now()
    run_dir_path = './run ({})'.format(now.strftime('%d-%m-%Y %H:%M'))
